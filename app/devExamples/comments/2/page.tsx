...
import { simpleComments, SimpleComment, simpleMarkdownContent } from '../../../../lib/devExamples/simpleCommentsTestData';
...
function commentsToHighlights(comments: SimpleComment[]) {
  return comments.map((comment) => ({
    ...
    quotedText: simpleMarkdownContent.substring(comment.highlightStart, comment.highlightEnd),
  ...
  const calculatePositions = useCallback(() => {
    ...
    simpleComments.forEach((comment) => {
      ...
      if (highlightElements.length > 0) {
        ...
        // Fallback position
        newPositions[comment.id] = comment.id === 'comment-1' ? 100 : comment.id === 'comment-2' ? 300 : 500;
      }
    });
    ...
    const getCommentHeight = (commentId: string) => {
      ...
      return baseHeight + (lines * lineHeight) + (comment.text.length > 200 ? 25 : 0); // Extra space for hover hint
    };
                    ...
                    >
                      ...
                      {needsTruncation && (
                        <div style={{
                          ...
                          {isHovered ? '↑ Full comment shown' : '↑ Hover to expand'}
                        </div>
                      )}
                    </div>
                  </div>
          ...
          <div className="w-80 bg-white rounded-lg shadow-sm p-6">
            ...
            <div className="space-y-2 text-sm">
              <div>
                ...
                <span className={highlightsReady ? 'text-green-600' : 'text-yellow-600'}>
                  {highlightsReady ? 'Yes' : 'No'}
                </span>
              </div>
              <div>
                <span className="font-medium">Positions calculated:</span>{' '}
                <span className={positionsCalculated ? 'text-green-600' : 'text-yellow-600'}>
                  {positionsCalculated ? 'Yes' : 'No'}
                </span>
              </div>
              <div className="mt-4">
                <div className="font-medium mb-2">Comment Positions:</div>
