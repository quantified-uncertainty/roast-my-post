...
// Helper to merge profile config with defaults
function buildMergedConfig(profile: Profile): ProfileConfig {
  return {
    ...DEFAULT_CONFIG,
    ...profile.config,
...
export function ProfileDetailView({ profile, onSave }: ProfileDetailViewProps) {
  ...
  const mergedConfig = buildMergedConfig(profile);
  ...
  useEffect(() => {
    ...
    setEditedConfig(buildMergedConfig(profile));
    setIsEditing(false);
