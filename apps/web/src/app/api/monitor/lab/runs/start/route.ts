  ...
  try {
    ...
    for (const documentId of documentIds) {
      try {
        ...
        logger.error("Failed to create job for document", { documentId, error });
        failedDocuments.push(documentId);
      }
    }

    if (jobIds.length === 0) {
      await metaEvaluationRepository.updateValidationRunStatus(run.id, "failed", "All jobs failed to create");
      return NextResponse.json(
        { error: "Failed to create any jobs", failedDocuments },
        { status: 500 }
      );
    }
