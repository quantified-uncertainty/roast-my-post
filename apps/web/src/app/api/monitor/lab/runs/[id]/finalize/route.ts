...
export async function POST(
  ...
  try {
    ...
    // TODO: This queries for the globally most recent EvaluationVersion per document,
    // not the versions created by this specific run. If another validation run completes
    // concurrently, we may pick up its versions instead. Fix by linking ValidationRun
    // to AgentEvalBatch and querying through the batch's jobs.
    ...
    for (const baselineSnapshot of baselineSnapshots) {
      ...
      if (!newSnapshot) {
        skippedCount++;
        logger.warn("No new snapshot found for baseline document", {
          runId,
          documentId: baselineSnapshot.documentId,
        });
        continue;
      }
      ...
      if (!baselineSnapshotRecord) {
        skippedCount++;
        logger.warn("No baseline snapshot record found", {
          runId,
    ...
    const summary = skippedCount > 0
      ? `${unchangedCount} unchanged, ${changedCount} changed, ${skippedCount} skipped`
