import { vi } from 'vitest';
import { GET } from '../route';
import { NextRequest } from 'next/server';
import { prisma } from '@roast/db';

// Mock dependencies
vi.mock('@roast/db', () => ({
  prisma: {
    agent: {
      findUnique: vi.fn(),
    },
    job: {
      findMany: vi.fn(),
    },
  },
  Prisma: {},
}));

vi.mock('@/infrastructure/auth/auth-helpers', () => ({
  authenticateRequestPassive: vi.fn().mockResolvedValue(undefined),
}));

vi.mock('@/infrastructure/auth/privacy-service', () => ({
  PrivacyService: {
    getEvaluationPrivacyFilter: vi.fn((userId?: string) => {
      if (!userId) {
        return { document: { isPrivate: false } };
      }
      return {
        OR: [
          { document: { isPrivate: false } },
          { document: { submittedById: userId } },
        ],
      };
    }),
  },
}));

describe('GET /api/evaluators/[agentId]/jobs', () => {
  const mockAgentId = 'agent-123';

  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('should return 404 for non-existent agent', async () => {
    (prisma.agent.findUnique as vi.MockedFunction<any>).mockResolvedValueOnce(null);

    const request = new NextRequest(`http://localhost:3000/api/agents/${mockAgentId}/jobs`);
    const response = await GET(request, { params: Promise.resolve({ agentId: mockAgentId }) });

    expect(response.status).toBe(404);
    const data = await response.json();
    expect(data.error).toBe('Agent not found');
  });

  it('should return only public jobs for anonymous users', async () => {
    (prisma.agent.findUnique as vi.MockedFunction<any>).mockResolvedValueOnce({
      id: mockAgentId
    });

    const mockJobs = [
      {
        id: 'job-1',
        status: 'completed',
        createdAt: new Date('2024-01-01'),
        startedAt: new Date('2024-01-01T00:01:00Z'),
        completedAt: new Date('2024-01-01T00:05:00Z'),
        error: null,
        priceInDollars: 0.10,
        durationInSeconds: 240,
        attempts: 1,
        originalJobId: null,
        evaluation: {
          id: 'eval-1',
          document: {
            id: 'doc-1',
            versions: [{ title: 'Document One' }]
          },
          agent: {
            id: 'agent-1',
            versions: [{ name: 'Agent One' }]
          }
        },
        evaluationVersion: {
          id: 'eval-version-1',
          grade: 85,
          summary: 'Good evaluation',
          comments: []
        },
        agentEvalBatch: null,
      },
    ];

    (prisma.job.findMany as vi.MockedFunction<any>).mockResolvedValueOnce(mockJobs);

    const request = new NextRequest(`http://localhost:3000/api/agents/${mockAgentId}/jobs`);
    const response = await GET(request, { params: Promise.resolve({ agentId: mockAgentId }) });

    expect(response.status).toBe(200);
    const data = await response.json();
    expect(data.jobs).toHaveLength(1);
    expect(data.jobs[0].id).toBe('job-1');

    // Verify privacy filter applied — anonymous users only see public docs
    expect(prisma.job.findMany).toHaveBeenCalledWith({
      where: {
        evaluation: {
          agentId: mockAgentId,
          document: { isPrivate: false },
        },
      },
      orderBy: { createdAt: 'desc' },
      take: 100,
      select: expect.any(Object),
    });
  });

  it('should filter by batchId when provided', async () => {
    (prisma.agent.findUnique as vi.MockedFunction<any>).mockResolvedValueOnce({
      id: mockAgentId
    });

    const batchId = 'batch-456';
    (prisma.job.findMany as vi.MockedFunction<any>).mockResolvedValueOnce([]);

    const request = new NextRequest(`http://localhost:3000/api/agents/${mockAgentId}/jobs?batchId=${batchId}`);
    const response = await GET(request, { params: Promise.resolve({ agentId: mockAgentId }) });

    expect(response.status).toBe(200);

    expect(prisma.job.findMany).toHaveBeenCalledWith({
      where: {
        evaluation: {
          agentId: mockAgentId,
          document: { isPrivate: false },
        },
        agentEvalBatchId: batchId,
      },
      orderBy: { createdAt: 'desc' },
      take: 100,
      select: expect.any(Object),
    });
  });

  it('should include private docs for authenticated users', async () => {
    const { authenticateRequestPassive } = await import('@/infrastructure/auth/auth-helpers');
    (authenticateRequestPassive as vi.MockedFunction<any>).mockResolvedValueOnce('user-123');

    (prisma.agent.findUnique as vi.MockedFunction<any>).mockResolvedValueOnce({
      id: mockAgentId
    });
    (prisma.job.findMany as vi.MockedFunction<any>).mockResolvedValueOnce([]);

    const request = new NextRequest(`http://localhost:3000/api/agents/${mockAgentId}/jobs`);
    const response = await GET(request, { params: Promise.resolve({ agentId: mockAgentId }) });

    expect(response.status).toBe(200);

    // Authenticated users see public + their own private docs
    expect(prisma.job.findMany).toHaveBeenCalledWith({
      where: {
        evaluation: {
          agentId: mockAgentId,
          OR: [
            { document: { isPrivate: false } },
            { document: { submittedById: 'user-123' } },
          ],
        },
      },
      orderBy: { createdAt: 'desc' },
      take: 100,
      select: expect.any(Object),
    });
  });

  it('should handle database errors', async () => {
    (prisma.agent.findUnique as vi.MockedFunction<any>).mockResolvedValueOnce({
      id: mockAgentId
    });

    (prisma.job.findMany as vi.MockedFunction<any>).mockRejectedValueOnce(new Error('Database error'));

    const request = new NextRequest(`http://localhost:3000/api/agents/${mockAgentId}/jobs`);
    const response = await GET(request, { params: Promise.resolve({ agentId: mockAgentId }) });

    expect(response.status).toBe(500);
    const data = await response.json();
    expect(data.error).toBe('Failed to fetch jobs');
  });
});
