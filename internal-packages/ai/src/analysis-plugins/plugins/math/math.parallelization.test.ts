...
const mockCheckMathHybrid = checkMathHybridTool as jest.Mocked<typeof checkMathHybridTool>;
...
describe('Math Plugin Parallelization Tests', () => {
  ...
  beforeEach(() => {
    ...
    mockCheckMathHybrid.execute = jest.fn().mockImplementation(async (input) => {
  ...
  describe('Parallel Execution Verification', () => {
    it('should execute math checks in parallel, not sequentially', async () => {
      ...
      extractMathExpressionsTool.execute = jest.fn()
    ...
    it('should maintain correct results despite parallel execution', async () => {
      ...
      const extractMathExpressionsTool = require('../../../tools/extract-math-expressions').extractMathExpressionsTool;
      extractMathExpressionsTool.execute = jest.fn().mockResolvedValue({
        expressions: [
          {
            originalText: '5+5=10',
            expression: '5+5',
  ...
  describe('Performance Comparison', () => {
    it('should demonstrate significant speedup with parallel execution', async () => {
      ...
      const extractMathExpressionsTool = require('../../../tools/extract-math-expressions').extractMathExpressionsTool;
      extractMathExpressionsTool.execute = jest.fn().mockResolvedValue({ expressions });

      // Each check takes 50ms
      mockCheckMathHybrid.execute = jest.fn().mockImplementation(async () => {
