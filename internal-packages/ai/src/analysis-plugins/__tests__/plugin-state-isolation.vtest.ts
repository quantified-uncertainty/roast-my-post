...
describe("Plugin State Isolation", () => {
  it("should not leak state between analyses", async () => {
    ...
    const result1 = await manager.analyzeDocument(doc1Text, {
      targetHighlights: 10,
    });
    ...
    const result2 = await manager.analyzeDocument(doc2Text, {
      targetHighlights: 10,
    });
  ...
  it("should handle multiple concurrent analyses without state pollution", async () => {
    const manager = new PluginManager({
      pluginSelection: {
        include: [PluginType.SPELLING],
      },
    ...
    const results = await Promise.all(
      documents.map((doc) => {
        return manager.analyzeDocument(doc.text, {
          targetHighlights: 10,
        });
      })
    );
    ...
    const doc1Comments = allComments[0]
      .map((c) => c.highlight?.quotedText)
      .filter(Boolean);
    const doc2Comments = allComments[1]
      .map((c) => c.highlight?.quotedText)
      .filter(Boolean);
    const doc3Comments = allComments[2]
      .map((c) => c.highlight?.quotedText)
      .filter(Boolean);
  ...
  it("should create fresh plugin instances for each analysis", async () => {
    const manager = new PluginManager({
      pluginSelection: {
        include: [PluginType.SPELLING],
      },
