...
export function calculateAnthropicCost(
  ...
  // Cache write cost - use TTL-specific breakdown if available
  let cacheWriteCost = 0;
  if (usage.cache_creation?.ephemeral_5m_input_tokens && pricing.cacheWrite5m) {
    cacheWriteCost += (usage.cache_creation.ephemeral_5m_input_tokens / 1_000_000) * pricing.cacheWrite5m;
  }
  if (usage.cache_creation?.ephemeral_1h_input_tokens && pricing.cacheWrite1h) {
    cacheWriteCost += (usage.cache_creation.ephemeral_1h_input_tokens / 1_000_000) * pricing.cacheWrite1h;
  }
  // Fall back to total cache_creation_input_tokens with 5m rate if no breakdown
  if (cacheWriteCost === 0 && usage.cache_creation_input_tokens && pricing.cacheWrite5m) {
    cacheWriteCost = (usage.cache_creation_input_tokens / 1_000_000) * pricing.cacheWrite5m;
  }
