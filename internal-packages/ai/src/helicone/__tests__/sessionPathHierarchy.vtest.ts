...
describe("Session Path Hierarchy", () => {
  ...
  it("builds correct path hierarchy for job -> analysis -> plugins -> tool", async () => {
    const paths: string[] = [];
  ...
  it("verifies actual plugin manager usage pattern", async () => {
    ...
    await jobSession.trackAnalysis("document", async () => {
      ...
      await pluginManagerSession.withPath("/plugins", undefined, async () => {
        ...
        await currentPluginManager.withPath(
          "/fact-check",
          { plugin: "fact-check" },
          async () => {
            capturedHeaders.push({
    ...
    const sessionIds = capturedHeaders.map(
      (h) => h.headers["Helicone-Session-Id"]
    );
    ...
    expect(capturedHeaders[2].headers["Helicone-Session-Path"]).toBe(
      "/analysis/document/plugins/fact-check"
    );
    expect(capturedHeaders[3].headers["Helicone-Session-Path"]).toBe(
      "/analysis/document/plugins/fact-check/tools/extract-claims"
    );
    expect(capturedHeaders[4].headers["Helicone-Session-Path"]).toBe(
      "/analysis/document/plugins/fact-check/tools/fact-checker"
    );
