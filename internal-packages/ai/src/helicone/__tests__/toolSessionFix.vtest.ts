...
describe("Tool Session Fix Verification", () => {
  ...
  it("check-math-with-mathjs tool now uses global session", () => {
    ...
    console.log(
      "✅ check-math-with-mathjs tool correctly uses job session:",
      sessionId
    );
  });

  it("tools create standalone sessions only when no global context", () => {
    ...
    console.log(
      "✅ Tool correctly creates standalone session when no global context:",
      sessionId
    );
  });

  it("full job -> plugin -> tool flow maintains session hierarchy", async () => {
    ...
    await jobManager.trackAnalysis("document", async () => {
      ...
      await jobManager.trackPlugin("math", async () => {
        ...
        const toolSessionId =
          getCurrentHeliconeHeaders()["Helicone-Session-Id"] ||
    ...
    expect(capturedHeaders[3].path).toBe(
    ...
    capturedHeaders.forEach((h) =>
      console.log(`  ${h.context}: ${h.sessionId} @ ${h.path}`)
    );
  });
});
