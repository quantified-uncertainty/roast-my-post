...
describe("HeliconeSessionManager", () => {
  ...
  describe("header generation", () => {
    ...
    it("includes properties as headers", () => {
      const manager = new HeliconeSessionManager({
        sessionId: "test-123",
        sessionName: "Test Session",
        properties: {
  ...
  describe("path tracking", () => {
    it("tracks nested paths correctly", async () => {
      const manager = new HeliconeSessionManager({
        sessionId: "test-123",
        sessionName: "Test Session",
      });
      ...
      await manager.withPath("/analysis", undefined, async () => {
        expect(getCurrentHeliconeHeaders()["Helicone-Session-Path"]).toBe(
          "/analysis"
        );
  ...
  describe("property stacking", () => {
    it("stacks properties with later values winning", async () => {
      const manager = new HeliconeSessionManager({
        sessionId: "test-123",
        sessionName: "Test Session",
        properties: {
      ...
      await manager.withPath(
        "/level1",
        { Level: "one", Extra: "value" },
        async () => {
          // Get the current global manager for nested call
          const currentManager = getGlobalSessionManager()!;

          await currentManager.withPath(
            "/level2",
            { Level: "two" },
            async () => {
  ...
  describe("header sanitization", () => {
    it("sanitizes non-ASCII characters", () => {
      const manager = new HeliconeSessionManager({
        sessionId: "test-123",
      ...
      const headers = manager.getHeaders();
      expect(headers["Helicone-Session-Name"]).toBe(
        'Test "Smart Quotes" - with em-dash'
      );
    });
  ...
  describe("convenience methods", () => {
    it("trackPlugin adds correct path and properties", async () => {
      const manager = new HeliconeSessionManager({
        sessionId: "test-123",
        sessionName: "Test Session",
      });
    ...
    it("trackTool adds correct path and properties", async () => {
      const manager = new HeliconeSessionManager({
        sessionId: "test-123",
        sessionName: "Test Session",
      });
    ...
    it("trackAnalysis adds correct path and properties", async () => {
      const manager = new HeliconeSessionManager({
        sessionId: "test-123",
        sessionName: "Test Session",
      });
  ...
  describe("forJob factory method", () => {
    it("creates job session with correct configuration", () => {
  ...
  describe("global session management", () => {
    it("sets and gets global session manager", () => {
    ...
    it("getCurrentHeliconeHeaders returns headers from global manager", () => {
  ...
  describe("error handling", () => {
    ...
    it("cleans up path stack on error", async () => {
      const manager = new HeliconeSessionManager({
        sessionId: "test-123",
        sessionName: "Test Session",
      });
  ...
  describe("plugin and tool session hierarchy", () => {
    it("tools should inherit session ID from global manager", async () => {
      // Create a job session manager and set it globally
      const jobManager = HeliconeSessionManager.forJob("job-123", "Test Job", {
        JobId: "job-123",
        AgentId: "agent-456",
      });
      ...
      await jobManager.trackPlugin("math", async () => {
        ...
        await jobManager.trackTool("check-math", async () => {
          ...
          expect(toolHeaders["Helicone-Session-Path"]).toBe(
            "/plugins/math/tools/check-math"
          );
    ...
    it("nested tool calls maintain session hierarchy", async () => {
      ...
      expect(capturedHeaders[0].headers["Helicone-Session-Path"]).toBe(
        "/analysis/document"
      );
      expect(capturedHeaders[1].headers["Helicone-Session-Path"]).toBe(
        "/analysis/document/plugins/forecast"
      );
      expect(capturedHeaders[2].headers["Helicone-Session-Path"]).toBe(
        "/analysis/document/plugins/forecast/tools/extract-forecasts"
      );
    });

    it("parallel plugin execution shares session ID but may have overlapping paths", async () => {
      const manager = HeliconeSessionManager.forJob("job-789", "Parallel Job", {
        JobId: "job-789",
      });
      ...
      expect(plugin1Headers[0]["Helicone-Session-Path"]).toMatch(
        /\/plugins\/(math|spelling)/
      );
      expect(plugin2Headers[0]["Helicone-Session-Path"]).toMatch(
        /\/plugins\/(math|spelling)/
      );
    });

    it("global session manager is cleared after job completion", () => {
      const manager = HeliconeSessionManager.forJob(
        "job-999",
        "Cleanup Test Job",
        { JobId: "job-999" }
    ...
    it("tools without global manager create standalone sessions", () => {
