...
describe("Session Hierarchy Integration Tests", () => {
  ...
  it("reproduces the issue: tools creating standalone sessions", async () => {
  ...
  it("correct behavior: tools inherit from job session", async () => {
    ...
    const sessionIds = capturedHeaders.map(
      (c) => c.headers["Helicone-Session-Id"]
    );
  ...
  it("identifies the fix: tools should use global session manager", async () => {
    // Set up job session
    const jobManager = HeliconeSessionManager.forJob("job-456", "Test Job", {
      JobId: "job-456",
    });
    setGlobalSessionManager(jobManager);
    ...
    const executeToolWithProperSession = async (toolName: string) => {
      ...
      if (globalManager["Helicone-Session-Id"]) {
        console.log(
          `Tool ${toolName} using global session: ${globalManager["Helicone-Session-Id"]}`
        );
      ...
      console.log(
        `Tool ${toolName} creating standalone session: ${standaloneId}`
      );
      return standaloneId;
    };
  ...
  it("ChunkRouter should maintain session hierarchy", async () => {
  ...
  it("BUG REPRODUCTION: tools create their own session IDs", async () => {
  ...
  it("plugin analyze method should maintain session", async () => {
    ...
    // Verify paths
    expect(headerCaptures[0]["Helicone-Session-Path"]).toBe(
      "/plugins/forecast"
    );
    expect(headerCaptures[1]["Helicone-Session-Path"]).toBe(
      "/plugins/forecast/tools/extract-forecasts"
    );
    expect(headerCaptures[2]["Helicone-Session-Path"]).toBe(
      "/plugins/forecast/tools/generate-forecast"
    );
  });
});
