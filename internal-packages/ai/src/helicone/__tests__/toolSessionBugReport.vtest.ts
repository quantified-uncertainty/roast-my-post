...
describe("Tool Session Bug Report and Fix", () => {
  ...
  it("PROBLEM: Tools create standalone sessions ignoring job context", () => {
  ...
  it("SOLUTION: Tools should use global session manager", async () => {
    ...
    const correctToolImplementation = async (toolName: string) => {
      ...
      if (globalManager) {
        ...
        console.log(
          `\nNo global session, tool '${toolName}' creates standalone:`,
          standaloneId
        );
        return standaloneId;
      }
  ...
  it("IMPLEMENTATION GUIDE: How to fix tools", () => {
  ...
  it("VERIFICATION: Correct session hierarchy", async () => {
    const jobManager = HeliconeSessionManager.forJob("job-999", "Test Job", {
      JobId: "job-999",
    });
    setGlobalSessionManager(jobManager);
    ...
    await jobManager.trackAnalysis("document", async () => {
      ...
      await jobManager.trackPlugin("math", async () => {
        capturedPaths.push(
          getCurrentHeliconeHeaders()["Helicone-Session-Path"]
        );
