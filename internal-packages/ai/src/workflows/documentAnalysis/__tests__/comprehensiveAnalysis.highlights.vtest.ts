...
Line 5 has the final content.`;
  ...
  it("all highlights from comprehensive analysis should become highlights", async () => {
    ...
    const adjustedRefs = adjustLineReferences(
      contentLineRefs,
      prependLineCount
    );
    ...
    console.log(
      `Generated ${analysisResult.outputs.highlightInsights.length} insights`
    );
    ...
    highlightResult.outputs.highlights.forEach((highlight, index) => {
      console.log(
        `Highlight ${index + 1}: "${highlight.description?.substring(0, 50) || "No description"}..."`
      );
      expect(highlight.description).toBe(
        analysisResult.outputs.highlightInsights[index].suggestedHighlight
      );
      expect(highlight.highlight).toBeDefined();
      expect(highlight.highlight?.startOffset).toBeGreaterThanOrEqual(0);
      expect(highlight.highlight?.endOffset).toBeGreaterThan(
        highlight.highlight?.startOffset || 0
      );
    });
  });

  it("handles line number mismatches with fuzzy matching", async () => {
    // Get the number of lines added by prepend
    const prependLineCount = getPrependLineCount(mockDocument);
  ...
  it("skips invalid highlights gracefully", async () => {
    ...
    expect(highlightResult.outputs.highlights).toHaveLength(2);
    expect(highlightResult.outputs.highlights[0].description).toBe(
      "This is line 1 with some content."
    );
    expect(highlightResult.outputs.highlights[1].description).toBe(
      "Line 3 contains important information."
    );
  });
});
