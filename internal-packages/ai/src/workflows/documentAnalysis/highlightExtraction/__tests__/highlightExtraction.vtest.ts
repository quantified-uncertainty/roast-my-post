...
Line 16: More content here.`,
  ...
  it("converts highlightInsights to highlights with correct positions", async () => {
    ...
    expect(firstHighlight.description).toContain(
      "Line 9: This line contains important information"
    );
    ...
    if (firstHighlight.highlight) {
      ...
      const { quotedText, isValid, startOffset, endOffset } =
    ...
    expect(secondHighlight.description).toContain(
      "Line 15: Content in a section"
    );
    expect(secondHighlight.highlight?.isValid).toBe(true);
  });

  it("handles case where line numbers don't match content but text can be found", async () => {
    const analysisData: ComprehensiveAnalysisOutputs = {
      summary: "Test summary",
      ...
      highlightInsights: [
        {
          ...
          suggestedHighlight:
            "Line 9: This line contains important information.", // But this text DOES exist
        },
      ],
    };
  ...
  it("handles case where line numbers and text don't match content", async () => {
    const analysisData: ComprehensiveAnalysisOutputs = {
      summary: "Test summary",
      analysis: "Test analysis",
      highlightInsights: [
  ...
  it("handles empty highlightInsights array", async () => {
  ...
  it("verifies line counting with prepend metadata", async () => {
