...
Cannot perform real-time symbolic algebra or step-by-step equation solving. MathJS requires numerical values - can't simplify "x + x = 2x" symbolically. Uses ${getModelDisplayName(MODEL_CONFIG.analysis)} model.

## Technical Details

- Two-stage verification: MathJS Agent → LLM Fallback (if needed)
- Uses ${getModelDisplayName(MODEL_CONFIG.analysis)} model for both stages
