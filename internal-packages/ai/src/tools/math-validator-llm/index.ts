...
const inputSchema = z.object({
  statement: z
    ...
    .max(1000)
    .describe(
      'A single mathematical statement to analyze (e.g., "2 + 2 = 4" or "The square root of 16 is 4")'
    ),
  context: z
    .string()
    .max(500)
    .optional()
    .describe("Additional context about the statement"),
  ...
  errorDetails: z
    .object({
      ...
      displayCorrection: z
        .string()
        .describe(
          'XML markup for displaying correction (e.g., "<r:replace from=\"45\" to=\"234\"/>")'
        ),
      expectedValue: z
        .string()
        .optional()
        .describe("The expected/correct value"),
      actualValue: z
        .string()
        .optional()
        .describe("The actual/incorrect value found in the statement"),
    })
    .optional()
    .describe(
      "Details about the error (only present when status is verified_false)"
    ),
}) satisfies z.ZodType<CheckMathOutput>;

export class CheckMathTool extends Tool<CheckMathInput, CheckMathOutput> {
  config = mathValidatorLLMConfig;
  ...
  async execute(
    ...
    context.logger.info(
      `[CheckMathTool] Analyzing mathematical statement: "${input.statement}"`
    );
    ...
    try {
      ...
      return {
        ...
        explanation:
          "Failed to analyze the mathematical statement due to a technical error.",
        reasoning: `Technical error occurred during analysis: ${error instanceof Error ? error.message : "Unknown error"}`,
      };
    }
  }

  private async analyzeMathStatement(
    input: CheckMathInput,
    context: ToolContext
  ): Promise<CheckMathOutput> {
