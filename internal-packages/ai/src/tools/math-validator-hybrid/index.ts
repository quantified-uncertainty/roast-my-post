...
> {
  ...
  async execute(
    input: CheckMathHybridInput,
    context: ToolContext
  ): Promise<CheckMathHybridOutput> {
    context.logger.info(
      `[CheckMathHybridTool] Analyzing statement: "${input.statement}"`
    );
    ...
    try {
      ...
      let finalStatus:
        ...
        | "verified_false"
        | "verified_warning"
        | "cannot_verify" = mathJsResult.status;
      ...
      if (mathJsResult.status === "cannot_verify") {
        context.logger.info(
          "[CheckMathHybridTool] MathJS could not verify, trying LLM for conceptual analysis..."
        );
      ...
      if (
        mathJsResult.status === "verified_false" &&
        mathJsResult.errorDetails?.actualValue &&
        mathJsResult.errorDetails?.expectedValue
      ) {
      ...
      return {
        ...
        verifiedBy,
        mathJsResult:
          mathJsResult.status !== "cannot_verify"
            ? {
                status: mathJsResult.status,
                explanation: mathJsResult.explanation,
                mathJsExpression:
                  mathJsResult.verificationDetails?.mathJsExpression,
      ...
      return {
        ...
        explanation:
          "Failed to verify the mathematical statement due to a technical error.",
        verifiedBy: "mathjs",
        toolsUsed: ["mathjs"],
      };
    }
  ...
  override async beforeExecute(
    ...
    context.logger.info(
      `[CheckMathHybridTool] Starting hybrid mathematical verification for statement: "${input.statement}"`
    );
  }

  override async afterExecute(
    output: CheckMathHybridOutput,
    context: ToolContext
  ): Promise<void> {
    context.logger.info(
      `[CheckMathHybridTool] Verification result: ${output.status} (verified by: ${output.verifiedBy})`
    );
  }
