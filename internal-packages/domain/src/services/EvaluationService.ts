...
import type { EvaluationRepositoryInterface } from '@roast/db';
...
export class EvaluationService {
    ...
    try {
      ...
      for (const agentId of request.agentIds) {
        try {
          ...
          this.logger.error('Failed to create evaluation in batch', {
            error,
            documentId: request.documentId,
            agentId
          });
      ...
      if (results.length > 0) {
        if (errors.length > 0) {
          this.logger.warn('Some evaluations failed to create', {
            documentId: request.documentId,
      ...
      this.logger.error('Error creating evaluations batch', {
        error,
        documentId: request.documentId,
  ...
  private async checkDocumentAccess(documentId: string, userId: string): Promise<Result<void, AppError>> {
    const hasAccess = await this.evaluationRepository.checkDocumentAccess(documentId, userId);
  ...
  private async checkAgentExists(agentId: string): Promise<Result<void, AppError>> {
    const exists = await this.evaluationRepository.checkAgentExists(agentId);
