...
import type { 
  ...
  DocumentEntity as RepositoryDocumentEntity,
  DocumentWithEvaluations as RepositoryDocumentWithEvaluations,
...
export class DocumentService {
  constructor(
    private readonly docRepo: DocumentRepositoryInterface,
    ...
    try {
      ...
      if (agentIds && agentIds.length > 0) {
        ...
        if (evaluationResult.isError()) {
          this.logger.warn('Failed to create some evaluations', {
            documentId: document.id,
            agentIds,
          ...
          this.logger.info('Evaluations created successfully', {
            documentId: document.id,
      ...
      this.logger.error('Error creating document', { error, userId, data });
      return Result.fail(
  ...
  async getDocumentForReader(
    ...
    try {
      const repoDoc = await this.docRepo.findWithEvaluations(docId, includeStale);
      ...
      const doc = this.convertToDocumentWithEvaluations(repoDoc);
      return Result.ok(doc);
    } catch (error) {
      this.logger.error('Error fetching document', { error, docId });
      return Result.fail(
  ...
  async getDocument(docId: string): Promise<Result<DocumentEntity, AppError>> {
    try {
      const repoDoc = await this.docRepo.findById(docId);
      ...
      const doc = this.convertToDomainEntity(repoDoc);
      return Result.ok(doc);
    } catch (error) {
      this.logger.error('Error fetching document', { error, docId });
      return Result.fail(
  ...
  async updateDocument(
    ...
    try {
      ...
      this.logger.error('Error updating document', { error, docId, userId });
      return Result.fail(
  ...
  async deleteDocument(
    ...
    try {
      ...
      this.logger.error('Error deleting document', { error, docId, userId });
      return Result.fail(
  ...
  async getUserDocuments(
    ...
    try {
      const repoDocs = await this.docRepo.findByUser(userId, limit);
      const documents = repoDocs.map(doc => this.convertToDocumentWithEvaluations(doc));
      return Result.ok(documents);
    } catch (error) {
      this.logger.error('Error fetching user documents', { error, userId });
      return Result.fail(
  ...
  async getAllDocuments(): Promise<Result<DocumentWithEvaluations[], AppError>> {
    try {
      const repoDocs = await this.docRepo.findAll();
      const documents = repoDocs.map(doc => this.convertToDocumentWithEvaluations(doc));
      return Result.ok(documents);
    } catch (error) {
      this.logger.error('Error fetching all documents', { error });
      return Result.fail(
  ...
  async getStatistics(): Promise<Result<any, AppError>> {
    try {
      ...
      this.logger.error('Error fetching document statistics', { error });
      return Result.fail(
  ...
  /**
   * Convert repository document entity to domain entity
   */
  private convertToDomainEntity(repoDoc: RepositoryDocumentEntity): DocumentEntity {
    return new DocumentEntity(
      repoDoc.id,
      repoDoc.title,
      repoDoc.content,
      repoDoc.author,
      repoDoc.publishedDate,
      repoDoc.url,
      repoDoc.platforms,
      repoDoc.submittedById,
      repoDoc.importUrl || undefined,
      repoDoc.ephemeralBatchId || undefined,
      repoDoc.createdAt,
      repoDoc.updatedAt,
      repoDoc.markdownPrepend
    );
  }

  /**
   * Convert repository document with evaluations to domain type
   */
  private convertToDocumentWithEvaluations(repoDoc: RepositoryDocumentWithEvaluations): DocumentWithEvaluations {
    return {
      ...repoDoc,
      importUrl: repoDoc.importUrl || undefined,
      ephemeralBatchId: repoDoc.ephemeralBatchId || undefined
    };
  }
}