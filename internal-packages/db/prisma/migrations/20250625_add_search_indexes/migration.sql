...
CREATE INDEX IF NOT EXISTS "idx_document_versions_title_lower" 
  ON "DocumentVersion" (LOWER("title"));

-- Partial index on content prefix for performance (first 1000 chars)
CREATE INDEX IF NOT EXISTS "idx_document_versions_content_prefix" 
  ON "DocumentVersion" (LOWER(SUBSTRING("content", 1, 1000)));

-- Agent name search
CREATE INDEX IF NOT EXISTS "idx_agent_versions_name_lower" 
  ON "AgentVersion" (LOWER("name"));

-- GIN indexes for array fields (PostgreSQL specific)
CREATE INDEX IF NOT EXISTS "idx_document_versions_authors_gin" 
  ON "DocumentVersion" USING GIN ("authors");

CREATE INDEX IF NOT EXISTS "idx_document_versions_platforms_gin" 
  ON "DocumentVersion" USING GIN ("platforms");

-- URL-based search indexes
CREATE INDEX IF NOT EXISTS "idx_document_versions_import_url" 
  ON "DocumentVersion" ("importUrl") 
  WHERE "importUrl" IS NOT NULL;

CREATE INDEX IF NOT EXISTS "idx_document_versions_urls_gin" 
  ON "DocumentVersion" USING GIN ("urls");

CREATE INDEX IF NOT EXISTS "idx_document_versions_intended_agents_gin" 
...
CREATE INDEX IF NOT EXISTS "idx_documents_published_date_desc" 
