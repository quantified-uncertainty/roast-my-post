...
echo "🚀 Phase 2: Parallel Execution (ROBUST - macOS)"
echo
...
if ! command -v parallel &> /dev/null; then
    ...
    brew install parallel 2>/dev/null || {
        echo "❌ Could not install GNU parallel"
        exit 1
    }
fi

# Check if coreutils is installed (for timeout command)
if ! command -v gtimeout &> /dev/null; then
    echo "⚠️  GNU coreutils not found. Installing..."
    brew install coreutils 2>/dev/null || {
        echo "❌ Could not install coreutils (needed for timeout)"
...
Start your analysis now."
    ...
    # Run with timeout (using gtimeout on macOS)
    gtimeout $MAX_TIME claude -p "$PROMPT" --max-turns 15 --allowedTools Read > "$OUTPUT_FILE" 2> "$ERROR_FILE"
    local EXIT_CODE=$?
    ...
    # Update completed count (simpler version for macOS)
...
echo "$TASKS" | parallel -j 4 --line-buffer run_task {#} {}
...
# Cleanup
rm -f outputs/completed_count.txt