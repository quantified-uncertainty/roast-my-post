...
${findingsBySeverity.critical.map(f => {
    let finding = `- Line ${f.line}: ${f.issue} (${f.category})`;
    if (f.sourceUrl) finding += `\n  Source URL: ${f.sourceUrl}`;
    else if (f.source) finding += `\n  Source: ${f.source}`;
    return finding;
}).join('\n')}

Major Issues (${findingsBySeverity.major.length}):
${findingsBySeverity.major.map(f => {
    let finding = `- Line ${f.line}: ${f.issue} (${f.category})`;
    if (f.sourceUrl) finding += `\n  Source URL: ${f.sourceUrl}`;
    else if (f.source) finding += `\n  Source: ${f.source}`;
    return finding;
...
${findingsBySeverity.minor.map(f => {
    let finding = `- Line ${f.line}: ${f.issue} (${f.category})`;
    if (f.sourceUrl) finding += `\n  Source URL: ${f.sourceUrl}`;
    else if (f.source) finding += `\n  Source: ${f.source}`;
    return finding;
...
- **CRITICAL**: When findings include source URLs or citations, you MUST preserve them in your report
- **IMPORTANT**: Include all "Source:" references from the findings in your report
