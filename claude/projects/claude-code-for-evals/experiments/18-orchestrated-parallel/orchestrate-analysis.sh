...
# Create a file to track completed tasks
echo "0" > "$OUTPUT_DIR/completed-count.txt"

# Function to update progress
update_progress() {
    local task_id=$1
    local status=$2
    local current=$(cat "$OUTPUT_DIR/completed-count.txt")
    echo $((current + 1)) > "$OUTPUT_DIR/completed-count.txt"
    echo "   [$((current + 1))/$TASK_COUNT] Task $task_id: $status"
}
...
cat "$OUTPUT_DIR/task-list.json" | jq -r '.[] | @json' | parallel -j "$MAX_PARALLEL" --eta '
    TASK=$(echo {} | jq -r .)
    ...
    if timeout $TIMEOUT_PER_TASK claude -p "$(cat $PROMPT_FILE)" > "$OUTPUT_FILE.raw" 2>&1; then
        ...
        update_progress "$TASK_ID" "✅ Complete"
    else
        echo "{\"error\": \"Task failed or timed out\", \"task\": $TASK}" > "$OUTPUT_FILE"
...
if node "$SCRIPT_DIR/lib/track-usage.js" report "$OUTPUT_DIR" 2>/dev/null; then
  echo
