        ...
        <table className="min-w-full divide-y divide-gray-300">
          <thead>
            <tr>
              ...
              <th className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Document</th>
          ...
          <tbody className="divide-y divide-gray-200 bg-white">
            {jobs.map((job) => (
              <tr key={job.id}>
                ...
                <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                  <Link href={`/docs/${job.evaluation.document.id}`} className="text-blue-600 hover:text-blue-800">
                    {job.evaluation.document.id.substring(0, 8)}...
                  </Link>
                </td>
                <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                  <Link href={`/agents/${job.evaluation.agent.id}`} className="text-blue-600 hover:text-blue-800">
                ...
                <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                  {job.costInCents ? `$${(job.costInCents / 100).toFixed(2)}` : "N/A"}
                </td>
              </tr>
            ))}
          </tbody>
