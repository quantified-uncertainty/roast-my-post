...
resolutionDate: "December 31, 2026"
isFuture: true
...
resolutionDate: "January 31, 2027"
isFuture: true
...
5. Note any stated probabilities and resolution dates
...
For each: originalText (exact), thinking (2-3 sentences), three scores, binary rewrite, probability, resolution date, isFuture
</output>`;
...
const inputSchema = z.object({
  ...
  minQualityThreshold: z
    ...
    .max(100)
    .optional()
    .describe("Minimum average score (across precision, verifiability, and importance) required for a forecast to be included. Forecasts with average scores below this threshold will be filtered out."),
...
> {
  ...
  async execute(
    ...
    return {
      ...
      totalFound: results.forecasts.length,
  ...
  private async extractAndScoreForecasts(
    ...
    const qualityInstruction = minQualityThreshold !== undefined 
              ...
              items: {
                ...
                properties: {
                  ...
                  resolutionDate: {
                    type: "string",
                    description:
                      "When the prediction can be resolved (e.g., '2025', 'end of 2024', 'next 5 years'). Use null if no timeframe specified.",
                  },
                  isFuture: {
                    type: "boolean",
                    description:
