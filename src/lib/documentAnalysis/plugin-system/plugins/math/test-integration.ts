...
`;
  ...
  console.log("Step 1: Running new analyze() method...");
  
  // Use the new single-method API
  try {
    ...
    console.log(`- LLM Interactions: ${result.llmInteractions.length}`);
    
    // Get debug info
    const debugInfo = plugin.getDebugInfo();
    console.log(`\nStep 2: Debug information:`);
    console.log(`- Potential findings: ${debugInfo.stats.potentialCount}`);
    console.log(`  - Math errors: ${debugInfo.stats.mathErrors}`);
    console.log(`  - Correct equations: ${debugInfo.stats.correctEquations}`);
    console.log(`- Located findings: ${debugInfo.stats.locatedCount}`);

    // Print errors found
    const errorFindings = debugInfo.findings.potential.filter((f: any) => f.type === 'math_error');
    ...
    console.log(`\nStep 3: Generated ${result.comments.length} comments:`);
    ...
    // Check success rate
    const investigated = debugInfo.stats.investigatedCount;
    const located = debugInfo.stats.locatedCount;
    const successRate = investigated > 0 ? (located / investigated) * 100 : 0;

    console.log(`\n\nSuccess Rate: ${located}/${investigated} = ${successRate.toFixed(1)}%`);
    
    console.log("\nDebug stats:", debugInfo.stats);
